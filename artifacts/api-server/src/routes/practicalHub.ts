...
function buildExaminerPersona(
  ...
  const anatomyImageBlock = anatomyImageCategory
    ? anatomyImageGroundTruth
      ...
      : `\nNo specimen image is currently available for this ${anatomyImageCategory} station. Do NOT pretend an image is displayed. Instead, conduct this as a spoken spotter-style viva: silently pick one well-known, classic ${anatomyImageCategory.toLowerCase()} specimen (including which side, if applicable) to use as the hidden ground truth for this station, but NEVER say its name, identity, or side out loud in your first question or at any point before the student attempts identification — use only a generic framing such as "Consider the dried specimen placed in front of you: identify it and its side, if any" and then question the student on it as if they were physically examining it, using your silently-chosen identity only to judge their answer and ask specimen-grounded follow-ups.`
    : "";
