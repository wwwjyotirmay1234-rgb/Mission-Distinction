...
const plan = {
  summary: weakSubjects.length > 0
    ? `Focus on your weak areas: ${weakSubjects.join(", ")} with daily revision and MCQ practice.`
    : "Maintain consistent revision of Anatomy, Physiology and Biochemistry with regular MCQ practice.",

  days: [
    {
      day: "Day 1",
      focus: weakSubjects[0] || "Anatomy",
      tasks: [
    ...
    {
      day: "Day 2",
      focus: weakSubjects[1] || "Physiology",
      tasks: [
    ...
    },
    {
      ...
      focus: "Biochemistry",
      tasks: [
        "Revise pathways and clinical correlations",
        "Practice MCQs",
...
for (const p of topPatterns.slice(0,5)) {
  if (p.acc !== null && p.acc < 60) {
    insights.push(
      `${p.tag} is a weak area. It appears frequently in PYQs. Revise this topic again.`
    );
  } else {
    insights.push(
      `${p.tag} is an important PYQ topic. Keep practising.`
    );
  }
}
