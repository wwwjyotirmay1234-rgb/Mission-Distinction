      ...
      {PARTICLES.map(p=>(
        <div key={p.id} className="absolute rounded-full pointer-events-none"
          style={{
            ...
            boxShadow:`0 0 ${p.size*2}px rgba(124,58,237,0.35)`,
            ...
            {BAR_HEIGHTS.map((h,i)=>(
              <div key={i} className="w-[5px] rounded-t-full"
                style={{
                  ...
                  boxShadow:active?"0 0 8px #7c3aed, 0 0 18px #7c3aed44":"none",
...
function AmbientSounds() {
  ...
  const toggle = (sound: typeof AMBIENT[0]) => {
    ...
    if (isOn) {
      ...
      if (node) {
        node.gain.gain.setTargetAtTime(0, getCtx().currentTime, 0.3);
          ...
          return (
            <div key={s.id} className="rounded-2xl border transition-all duration-300 overflow-hidden"
              style={{
                ...
                borderColor:on?`${s.color}55`:"rgba(255,255,255,0.07)",
              ...
              <button className="w-full p-3 flex flex-col items-center gap-1.5" onClick={()=>toggle(s)}>
                <div className="relative text-2xl">
                  {s.icon}
                  {on&&<div className="absolute -inset-1 rounded-full opacity-40 blur-sm pointer-events-none"
                    style={{background:s.color,animation:"gp 1.5s ease-in-out infinite"}} />}
                </div>
                  ...
                  <div className="flex items-end gap-0.5 h-3.5">
                    {[...Array(5)].map((_,i)=>(
                      <div key={i} className="w-[3px] rounded-full"
                        style={{background:s.color,height:"50%",animation:`wf ${.4+i*.12}s ease-in-out ${i*.08}s infinite`}} />
                    ))}
                  </div>
                )}
              </button>
...
export default function StudentMusic() {
  ...
  return (
    <div className="flex flex-col min-h-[calc(100vh-4rem)] -mx-4 -mt-4"
      ...
      <div className="flex gap-1 px-4 pt-3 pb-1">
        {tabs.map(t=>(
          <button key={t.id} onClick={()=>setTab(t.id)}
            ...
            style={tab===t.id?{background:"linear-gradient(135deg,#7c3aed,#6d28d9)",boxShadow:"0 2px 12px #7c3aed44",color:"white"}
            ...
            <div className="space-y-1.5">
              {loading&&(
                <div className="flex items-center justify-center py-10 gap-2 text-muted-foreground">
                  <Loader2 size={18} className="animate-spin text-primary"/><span className="text-sm">Searching…</span>
                </div>
              )}
                    ...
                    <div className="w-5 shrink-0 text-center">
                      {isNow?(
                        <div className="flex items-end gap-0.5 h-4 mx-auto w-fit">
                          {[...Array(3)].map((_,j)=>(<div key={j} className="w-[3px] rounded-full bg-primary" style={{height:"60%",animation:`wf ${.35+j*.1}s ease-in-out ${j*.08}s infinite`}} />))}
                        </div>
                ...
                {playlist.map((item,i)=>{
                  ...
                  return (
                    <div key={item.id}
                      ...
                      <img src={item.artwork} alt={item.trackName} className="w-11 h-11 rounded-xl object-cover shrink-0"/>
                      <div className="flex-1 min-w-0">
      ...
      {tab==="spotify"&&(
        <div className="px-4 pt-3 space-y-4">
          <p className="text-xs text-muted-foreground">Paste any Spotify track, album, or playlist link</p>
          <div className="flex gap-2">
            <input value={spotifyInput} onChange={e=>setSpotifyInput(e.target.value)}
              ...
              className="flex-1 h-10 rounded-xl bg-card/40 border border-border/60 px-3 text-sm text-foreground placeholder:text-muted-foreground focus:outline-none focus:ring-2 focus:ring-green-500/40"/>
          ...
          <div>
            <p className="text-xs font-bold text-muted-foreground uppercase tracking-wider mb-2">Study playlists</p>
            <div className="grid grid-cols-2 gap-2">
              {SPOTIFY_SUGGESTIONS.map(s=>(
                <button key={s.label} onClick={()=>loadSpotify(s.url)}
                  className="text-xs px-3 py-2.5 rounded-xl border border-green-500/15 bg-green-500/8 text-green-400 hover:bg-green-500/15 transition-all hover:scale-[1.02] text-left font-semibold">
                  {s.label}
                </button>
              ))}
            </div>
          </div>
          <p className="text-xs text-muted-foreground text-center">Free Spotify: 30-sec previews · Premium: full songs</p>
        </div>
      )}
