...
export function AdminSidebar() {
  ...
  return (
    <aside className="w-64 h-screen bg-sidebar border-r border-sidebar-border flex flex-col fixed left-0 top-0 z-20">
      <div className="p-6 flex items-center gap-3">
        ...
        <span className="font-bold text-lg text-foreground tracking-tight">Admin<span className="text-secondary">Portal</span></span>
      </div>
        ...
        {navItems.map((item) => {
          ...
          return (
            <Link key={item.href} href={item.href}>
              ...
              >
                <item.icon size={18} className={isActive ? "text-secondary-foreground" : "text-muted-foreground"} />
                {item.label}
              </div>
            </Link>
          );
