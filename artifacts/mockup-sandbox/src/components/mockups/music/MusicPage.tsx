...
import {
  ...
  SkipForward, SkipBack, Sparkles, Youtube, Radio
} from "lucide-react";
...
type Tab = "search" | "playlist" | "spotify" | "youtube" | "soundcloud";
...
const YT_SUGGESTIONS = [
  { label: "Lofi Girl – 24/7",        url: "https://www.youtube.com/watch?v=jfKfPfyJRdk" },
  { label: "Study Music Alpha Waves", url: "https://www.youtube.com/watch?v=WPni755-Krg" },
  { label: "Deep Focus – 3 Hours",    url: "https://www.youtube.com/watch?v=UrO8GVwzod0" },
  { label: "Rain & Jazz",             url: "https://www.youtube.com/watch?v=DVrBgKgpKTo" },
  { label: "Hans Zimmer Mix",         url: "https://www.youtube.com/watch?v=dh8zB2JWlSY" },
  { label: "Piano Study Music",       url: "https://www.youtube.com/watch?v=9EqHJqE7CAs" },
];
const SC_SUGGESTIONS = [
  { label: "Lofi Hip Hop",     url: "https://soundcloud.com/lo-fi-hip-hop-music/sets/chill-study-music" },
  { label: "Chillhop Essentials", url: "https://soundcloud.com/chillhopdotcom/sets/chillhop-essentials-spring-2024" },
  { label: "Study Beats",      url: "https://soundcloud.com/user-952027341/sets/study-beats" },
  { label: "Focus Flow",       url: "https://soundcloud.com/ambientmusic/sets/focus" },
];
const AMBIENT = [
...
function fmtTime(s: number) { if (!s||!isFinite(s)) return "0:00"; return `${Math.floor(s/60)}:${String(Math.floor(s%60)).padStart(2,"0")}`; }
function fmtMs(ms: number) { const s=Math.floor(ms/1000); return `${Math.floor(s/60)}:${String(s%60).padStart(2,"0")}`; }
...
function extractYouTubeId(input: string): string|null { const m=input.match(/(?:v=|youtu\.be\/|embed\/|shorts\/)([A-Za-z0-9_-]{11})/); return m?m[1]:null; }
function extractSoundCloudEmbed(url: string): string|null { if(!url.includes("soundcloud.com")) return null; return `https://w.soundcloud.com/player/?url=${encodeURIComponent(url)}&color=%237c3aed&auto_play=true&hide_related=false&show_comments=false&show_user=true&show_reposts=false&show_teaser=false&visual=true`; }
function loadLS<T>(key: string, fallback: T): T { try { return JSON.parse(localStorage.getItem(key)??"null")??fallback; } catch { return fallback; } }
...
function HeroSection({ active, query, setQuery, onSearch, loading }: { active:boolean; query:string; setQuery:(q:string)=>void; onSearch:(q:string)=>void; loading:boolean; }) {
  return (
    <div className="relative overflow-hidden px-5 pt-7 pb-8" style={{background:"linear-gradient(135deg,#0a0c18 0%,#150a32 35%,#1e0d4a 55%,#0d0f1a 100%)"}}>
      ...
      <div className="relative mt-5">
        ...
        <input value={query} onChange={e=>setQuery(e.target.value)} onKeyDown={e=>{if(e.key==="Enter")onSearch(query);}} placeholder="Search any song, artist, album…" className="w-full h-12 pl-10 pr-28 rounded-2xl text-sm text-white placeholder:text-white/30 focus:outline-none focus:ring-2 focus:ring-violet-500/50" style={{background:"rgba(255,255,255,0.06)",border:"1px solid rgba(255,255,255,0.1)",backdropFilter:"blur(8px)"}}/>
        <div className="absolute right-2 top-1/2 -translate-y-1/2 flex items-center gap-1.5">
          ...
          <button onClick={()=>onSearch(query)} disabled={loading||!query.trim()} className="h-8 px-3 rounded-xl text-white text-xs font-semibold disabled:opacity-40 flex items-center gap-1.5 transition-all hover:scale-105" style={{background:"linear-gradient(135deg,#7c3aed,#6d28d9)",boxShadow:"0 2px 12px #7c3aed55"}}>{loading?<Loader2 size={12} className="animate-spin"/>:<Sparkles size={12}/>} Search</button>
        </div>
      </div>
    </div>
  );
...
function FloatingPlayer({ previewUrl,trackName,artistName,artwork,onSkipPrev,onSkipNext,canPrev,canNext,onClose }:{previewUrl:string;trackName:string;artistName:string;artwork:string;onSkipPrev:()=>void;onSkipNext:()=>void;canPrev:boolean;canNext:boolean;onClose:()=>void;}) {
  const audioRef=useRef<HTMLAudioElement>(null);
  const [playing,setPlaying]=useState(false),[progress,setProgress]=useState(0),[currentT,setCurrentT]=useState(0),[duration,setDuration]=useState(0),[buffering,setBuffering]=useState(false);
  useEffect(()=>{if(!audioRef.current||!previewUrl)return;audioRef.current.src=previewUrl;audioRef.current.load();setProgress(0);setCurrentT(0);setDuration(0);setBuffering(true);audioRef.current.play().then(()=>{setPlaying(true);setBuffering(false);}).catch(()=>setBuffering(false));},[previewUrl]);
  ...
  const seek=(e:React.MouseEvent<HTMLDivElement>)=>{if(!audioRef.current||!duration)return;const rect=e.currentTarget.getBoundingClientRect();audioRef.current.currentTime=((e.clientX-rect.left)/rect.width)*duration;};
  return (
    <div className="fixed bottom-0 left-0 right-0 z-50" style={{background:"rgba(13,15,26,0.97)",backdropFilter:"blur(20px)",borderTop:"1px solid rgba(255,255,255,0.08)",boxShadow:"0 -8px 32px rgba(0,0,0,0.6)"}}>
      <audio ref={audioRef} onTimeUpdate={()=>{if(!audioRef.current)return;const d=audioRef.current.duration||0,c=audioRef.current.currentTime;setCurrentT(c);setDuration(d);setProgress(d?c/d:0);}} onEnded={()=>{setPlaying(false);setProgress(1);}} onWaiting={()=>setBuffering(true)} onCanPlay={()=>setBuffering(false)} onError={()=>{setPlaying(false);setBuffering(false);}} crossOrigin="anonymous"/>
      <div className="w-full h-1 bg-white/10 cursor-pointer" onClick={seek}><div className="h-full rounded-full" style={{width:`${progress*100}%`,background:"linear-gradient(90deg,#6d28d9,#a78bfa)"}}/></div>
      <div className="flex items-center gap-3 px-4 py-3">
        <div className="relative shrink-0"><img src={artwork} alt="" className="w-10 h-10 rounded-xl object-cover" style={{boxShadow:"0 0 12px rgba(124,58,237,.45)"}}/><div className="absolute -inset-0.5 rounded-xl border border-violet-500/35 pointer-events-none"/>{playing&&<div className="absolute inset-0 rounded-xl flex items-end justify-center pb-1 gap-0.5 bg-black/40">{[...Array(4)].map((_,i)=><div key={i} className="w-[2px] rounded-full bg-violet-400" style={{height:"55%",animation:`wf ${.3+i*.1}s ease-in-out ${i*.05}s infinite`}}/>)}</div>}</div>
        <div className="flex-1 min-w-0"><p className="text-sm font-semibold truncate" style={{background:"linear-gradient(90deg,#c4b5fd,#a78bfa)",WebkitBackgroundClip:"text",WebkitTextFillColor:"transparent"}}>{trackName}</p><div className="flex items-center gap-2 mt-0.5"><p className="text-[11px] text-white/40 truncate">{artistName}</p><span className="text-[10px] text-white/25 shrink-0">{fmtTime(currentT)} / {fmtTime(duration)}</span></div></div>
        <div className="flex items-center gap-1 shrink-0">
          ...
          <button onClick={togglePlay} className="p-2.5 rounded-full text-white transition-all hover:scale-110" style={{background:"linear-gradient(135deg,#7c3aed,#6d28d9)",boxShadow:"0 2px 12px #7c3aed66"}}>{buffering?<Loader2 size={14} className="animate-spin"/>:playing?<Pause size={14}/>:<Play size={14}/>}</button>
...
function YouTubeTab() {
  const [input,setInput]=useState(""),[videoId,setVideoId]=useState<string|null>(null),[error,setError]=useState(""),[audioOnly,setAudioOnly]=useState(false);
  const load=(url:string)=>{setError("");const id=extractYouTubeId(url);if(!id){setError("Paste a valid YouTube link.");return;}setVideoId(id);setInput(url);};
  return (
    <div className="px-4 pt-3 space-y-4">
      <div className="p-3 rounded-2xl border border-red-500/15 bg-red-500/5 flex items-start gap-2"><Youtube size={14} className="text-red-400 mt-0.5 shrink-0"/><p className="text-[11px] text-white/60 leading-relaxed">Paste any YouTube link to play it here. Full videos and livestreams both work. Toggle <span className="text-white/80 font-semibold">Audio Only</span> to hide video and save battery.</p></div>
      <div className="flex gap-2"><input value={input} onChange={e=>setInput(e.target.value)} onKeyDown={e=>{if(e.key==="Enter")load(input);}} placeholder="https://www.youtube.com/watch?v=…" className="flex-1 h-10 rounded-xl px-3 text-xs text-white placeholder:text-white/25 focus:outline-none focus:ring-2 focus:ring-red-500/40" style={{background:"rgba(255,255,255,0.05)",border:"1px solid rgba(255,255,255,0.08)"}}/><button onClick={()=>load(input)} className="h-10 px-4 rounded-xl text-white text-xs font-semibold shrink-0 transition-all hover:scale-105" style={{background:"linear-gradient(135deg,#dc2626,#b91c1c)",boxShadow:"0 2px 10px #dc262644"}}>Load</button></div>
      {error&&<p className="text-xs text-red-400">{error}</p>}
      {videoId&&(
        <div className="space-y-2">
          <div className="flex items-center justify-between"><span className="text-xs text-white/40">Now playing</span><button onClick={()=>setAudioOnly(v=>!v)} className={`flex items-center gap-1.5 text-xs px-3 py-1.5 rounded-xl border transition-all ${audioOnly?"bg-violet-500/20 border-violet-500/30 text-violet-300":"bg-white/5 border-white/10 text-white/40"}`}><Volume2 size={11}/> {audioOnly?"Audio only (on)":"Audio only"}</button></div>
          {!audioOnly?(
            <div className="relative rounded-2xl overflow-hidden" style={{paddingBottom:"56.25%",height:0}}><iframe key={videoId} src={`https://www.youtube-nocookie.com/embed/${videoId}?autoplay=1&rel=0&modestbranding=1`} className="absolute inset-0 w-full h-full border-0" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" allowFullScreen title="YouTube player"/></div>
          ):(
            <div className="rounded-2xl border border-violet-500/20 bg-violet-500/8 p-5 flex flex-col items-center gap-3"><div className="w-14 h-14 rounded-2xl bg-violet-500/20 flex items-center justify-center"><Volume2 size={24} className="text-violet-400"/></div><div className="text-center"><p className="text-sm font-semibold text-white/80">Audio playing in background</p><p className="text-xs text-white/35 mt-1">Video hidden to save battery</p></div><iframe key={`audio-${videoId}`} src={`https://www.youtube-nocookie.com/embed/${videoId}?autoplay=1&rel=0`} className="w-0 h-0 border-0 absolute opacity-0 pointer-events-none" allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture" title="YouTube audio"/></div>
          )}
          <button onClick={()=>{setVideoId(null);setInput("");}} className="w-full text-xs text-white/25 hover:text-white/50 transition-colors py-1">✕ Close player</button>
        </div>
      )}
      <div><p className="text-[11px] font-bold text-white/30 uppercase tracking-wider mb-2">Study channels & livestreams</p><div className="grid grid-cols-2 gap-2">{YT_SUGGESTIONS.map(s=><button key={s.label} onClick={()=>load(s.url)} className="text-[11px] px-3 py-2.5 rounded-xl border border-red-500/15 bg-red-500/5 text-red-300 hover:bg-red-500/12 transition-all hover:scale-[1.02] text-left font-semibold leading-tight">▶ {s.label}</button>)}</div></div>
    </div>
  );
}
...
function SoundCloudTab() {
  ...
  const load=(url:string)=>{setError("");const e=extractSoundCloudEmbed(url);if(!e){setError("Paste a valid SoundCloud link.");return;}setEmbed(e);setInput(url);};
  return (
    <div className="px-4 pt-3 space-y-4">
      <div className="p-3 rounded-2xl border border-orange-500/15 bg-orange-500/5 flex items-start gap-2"><Radio size={14} className="text-orange-400 mt-0.5 shrink-0"/><p className="text-[11px] text-white/60 leading-relaxed">SoundCloud has millions of <span className="text-white/80 font-semibold">free full-length tracks</span> — lo-fi, classical, ambient and more. Paste any track, album or playlist link.</p></div>
      <div className="flex gap-2"><input value={input} onChange={e=>setInput(e.target.value)} onKeyDown={e=>{if(e.key==="Enter")load(input);}} placeholder="https://soundcloud.com/artist/track…" className="flex-1 h-10 rounded-xl px-3 text-xs text-white placeholder:text-white/25 focus:outline-none focus:ring-2 focus:ring-orange-500/40" style={{background:"rgba(255,255,255,0.05)",border:"1px solid rgba(255,255,255,0.08)"}}/><button onClick={()=>load(input)} className="h-10 px-4 rounded-xl text-white text-xs font-semibold shrink-0 transition-all hover:scale-105" style={{background:"linear-gradient(135deg,#ea580c,#c2410c)",boxShadow:"0 2px 10px #ea580c44"}}>Load</button></div>
      {error&&<p className="text-xs text-orange-400">{error}</p>}
      {embed&&<div className="space-y-2"><iframe key={embed} width="100%" height="300" scrolling="no" frameBorder="no" allow="autoplay" src={embed} className="rounded-2xl" title="SoundCloud Player"/><button onClick={()=>{setEmbed(null);setInput("");}} className="w-full text-xs text-white/25 hover:text-white/50 transition-colors py-1">✕ Close player</button></div>}
      <div><p className="text-[11px] font-bold text-white/30 uppercase tracking-wider mb-2">Suggested playlists</p><div className="grid grid-cols-2 gap-2">{SC_SUGGESTIONS.map(s=><button key={s.label} onClick={()=>load(s.url)} className="text-[11px] px-3 py-2.5 rounded-xl border border-orange-500/15 bg-orange-500/5 text-orange-300 hover:bg-orange-500/12 transition-all hover:scale-[1.02] text-left font-semibold leading-tight">▶ {s.label}</button>)}</div><p className="text-[10px] text-white/20 text-center mt-3">SoundCloud free = unlimited full songs · No login needed for most tracks</p></div>
    </div>
  );
}
...
export function MusicPage() {
  ...
  const [query,setQuery]=useState(""),[results,setResults]=useState<Track[]>([]),[loading,setLoading]=useState(false),[error,setError]=useState(""),[hasSearched,setHasSearched]=useState(false),[addedIds,setAddedIds]=useState<Set<number>>(new Set()),[nowPlaying,setNowPlaying]=useState<Track|null>(null),[nowIdx,setNowIdx]=useState(-1);
  ...
  const [plNowPlaying,setPlNowPlaying]=useState<PlaylistItem|null>(null),[plNowIdx,setPlNowIdx]=useState(-1);
  ...
  const eqActive=hasSearched||tab==="youtube"||tab==="soundcloud";
  const tabs:[Tab,string,string][]=[["search","Search","#7c3aed"],["playlist",playlist.length?`Playlist (${playlist.length})`:"Playlist","#7c3aed"],["spotify","Spotify","#16a34a"],["youtube","YouTube","#dc2626"],["soundcloud","SoundCloud","#ea580c"]];
  const activeFP=tab==="search"&&!!nowPlaying, activePlFP=tab==="playlist"&&!!plNowPlaying;
  return (
    <div className="min-h-screen bg-[#0d0f1a] text-white flex flex-col" style={{paddingBottom:(activeFP||activePlFP)?"84px":"16px"}}>
      ...
      <div className="flex gap-1.5 px-4 pt-3 pb-1 overflow-x-auto" style={{scrollbarWidth:"none"}}>
        {tabs.map(([id,label,color])=><button key={id} onClick={()=>setTab(id)} className="flex items-center px-3 py-2 rounded-xl text-xs font-semibold transition-all whitespace-nowrap shrink-0" style={tab===id?{background:`${color}cc`,boxShadow:`0 2px 12px ${color}55`,color:"white"}:{background:"rgba(255,255,255,0.05)",color:"rgba(255,255,255,.4)"}}>{label}</button>)}
      </div>
