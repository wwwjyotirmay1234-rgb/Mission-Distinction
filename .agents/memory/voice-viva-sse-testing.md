...
Also confirmed: the frontend UI itself gets stuck on "Connecting to the examiner..." text in the Playwright testing subagent's headless browser because `streamTurn()` awaits `playback.init()` (AudioContext + audioWorklet.addModule) BEFORE flipping state past "connecting" — that await hangs in headless/fake-audio-device Chromium. Reproduces identically on the already-shipped, previously-verified Physiology station, so it's a harness limitation, not a per-feature regression. Cross-check any new voice-viva station (e.g. a new subject/image) the same curl-first way rather than expecting `runTest()` to reach the "listening"/question UI.
